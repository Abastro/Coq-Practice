...
Proof.
  ...
  simpl in *. firstorder. left. apply in_map. assumption.
Qed.
...
Proof.
  ...
  simpl in *. firstorder. left.
  apply in_map_iff in H as [x [E H]]. apply INJ in E; subst. assumption.
Qed.
...
Proof with (auto with datatypes).
  ...
  - (* Repeats (l1 ++ l2) *)
    induction l1 as [| s l1' IH]; simpl in *...
    firstorder. apply in_app_iff in H as []...
Qed.
...
Proof with auto.
  ...
  apply incl_cons_inv in H as [[n ?] %In_nth_error  [ln ?] %IHl1].
  exists (n :: ln). simpl. f_equal...
Qed.
...
Proof with (auto with datatypes).
  assert (WTS: forall N,
    ...
    assert (REC: forall m l1 l2, l = l1 ++ m :: l2 -> ~In N' (l1 ++ l2) -> Repeats l). {
      ...
      apply Forall_and... apply Forall_forall.
...
Proof with (auto with datatypes).
  intros X l1 l2 [ln H] %mk_index_list HL.
  apply repeat_injection with (f := Some). { red. intros. injection H0... }
